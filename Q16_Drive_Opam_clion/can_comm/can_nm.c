...
static void send_nm_message(uint8_t nm_type)
{
    ...
    ((nm_message_t *)(CANTxBindData(GetCanNMTxInstance())))->source_node_id = g_nm_manager.local_node_id;
    ((nm_message_t *)(CANTxBindData(GetCanNMTxInstance())))->active_nodes = g_nm_manager.active_node_count;
    ((nm_message_t *)(CANTxBindData(GetCanNMTxInstance())))->nm_data = 0;
    /* 使用标准函数发送 */
    g_nm_stats.messages_sent++;

    CAN_NM_PRINTF("NM message sent: type=0x%02X, node=%d, active=%d\n", nm_type, g_nm_manager.local_node_id,
...
{
    /* 更新活跃节点信息 */
    if (msg->source_node_id < CAN_NM_MAX_NODES && msg->source_node_id != g_nm_manager.local_node_id)
    {
...
static void handle_nm_sleep_ind_message(const nm_message_t *msg)
{
    ...
    {
        /* 如果本节点也准备睡眠，发送应答 */
        if (g_nm_manager.sleep_condition_callback != NULL && g_nm_manager.sleep_condition_callback())
        {
...
static void update_node_status(uint32_t current_time)
{
    ...
    for (int i = 0; i < CAN_NM_MAX_NODES; i++)
    {
        ...
        /* 检查节点超时 */
        if (g_nm_manager.nodes[i].is_active && current_time - g_nm_manager.nodes[i].last_seen_time > CAN_NM_TIMEOUT_MS)
        {
...
{
    /* 检查唤醒条件 */
    if (g_nm_manager.wakeup_condition_callback != NULL && g_nm_manager.wakeup_condition_callback())
    {
        // CAN_NM_PRINTF("NM: Wakeup condition detected, transitioning to normal operation\n");
        g_nm_manager.current_state = NM_STATE_NORMAL_OPERATION;
        g_nm_stats.state_changes++;
...
static void handle_nm_prepare_sleep_state(uint32_t current_time)
{
    ...
    else if (current_time - g_nm_manager.prepare_start_time > CAN_NM_PREPARE_TIMEOUT_MS)
    {
...
{
    if (!g_nm_manager.is_initialized || data == NULL || len < sizeof(nm_message_t))
    {
        return;
    }
    ...
    /* 更新节点状态 */
    if (nm_msg->source_node_id < CAN_NM_MAX_NODES && nm_msg->source_node_id != g_nm_manager.local_node_id)
    {
...
/**
 * @brief 获取CAN NM接收实例
 * @return 返回指向can_comm_rx_t类型的指针，如果实例存在则返回该实例的指针，否则返回NULL
 */
...
/**
 * @brief 执行CAN网络管理任务
 * @details 该函数定期检查并显示当前的网络状态，包括总线状态、活跃节点数和总线负载。
