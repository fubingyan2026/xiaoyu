...
static const uint8_t protocol_len = sizeof(can_comm_protocol_head_t) + sizeof(can_comm_protocol_end_t);
...
static uint8_t floor_to_std(const uint8_t input_value)
{
    ...
    for (int i = sizeof(STANDARD_VALUES) / sizeof(STANDARD_VALUES[0]) - 1; i >= 0; --i)
    {
...
{
    /* 验证配置有效性 */
    if (!validate_can_rx_config(config))
        return NULL;
    /* 检查是否已存在同名实例 */
    ...
    /* 初始化环形缓冲区 */
    new_instance->kfifo_ptr = kfifo_alloc((config->rx_data_len + protocol_len) * CAN_FIFO_MULTIPLIER, NULL);
    ...
    const daemon_init_config_t daemon_config = {
        ...
        .reload_time_out = config->offline_ms > 0 ? config->offline_ms : CAN_DEFAULT_OFFLINE_MS,
        .init_wait_time = 200,
    };
    ...
    {
        CAN_COMM_PRINTF("Failed to allocate direct_binding_ptr for CAN RX instance %s\n", config->name);
...
{
    /* 验证配置有效性 */
    if (!validate_can_tx_config(config))
        return NULL;
    /* 检查是否已存在同名实例 */
    ...
    /* 初始化环形缓冲区 */
    new_instance->kfifo_ptr = kfifo_alloc((config->tx_data_len + protocol_len) * CAN_FIFO_MULTIPLIER, NULL);
    if (new_instance->kfifo_ptr == NULL)
    {
    ...
    {
        CAN_COMM_PRINTF("Failed to allocate direct_tx_data_ptr for CAN TX instance %s\n", config->name);
        if (new_instance->kfifo_ptr != NULL)
            kfifo_free(new_instance->kfifo_ptr);
        __free(new_instance);
        return NULL;
    ...
    while (*current != NULL)
    {
        if (*current == instance)
        {
            *current = instance->next;
            /* 释放环形缓冲区 */
            if (instance->kfifo_ptr != NULL)
                kfifo_free(instance->kfifo_ptr);
            /* 注销守护进程 */
...
void CANCommGetDataTransmit_V2(void)
{
    ...
    while (current != NULL)
    {
        ...
        const uint16_t total_packet_len =
            current->config.rx_data_len + sizeof(can_comm_protocol_head_t) + sizeof(can_comm_protocol_end_t);
        ...
        while (1)
        {
            ...
            if (p_head->start != 'S' || p_head->data_length != current->config.rx_data_len)
            {
            ...
            {
                kfifo_skip(current->kfifo_ptr, 1); // 尾部不对，说明可能只是数据里恰好有个 'S'
                continue;
            }

            // 5. 校验 CRC (这是最耗时的步骤，放在最后做)
            uint8_t calculated_crc = get_CRC8_check_sum(temp_buffer + header_len, current->config.rx_data_len, 0);
            ...
            if (current->direct_binding_ptr != NULL)
            {
                ...
                __memcpy((void *)current->direct_binding_ptr, temp_buffer + header_len, current->config.rx_data_len);
            }
        }
        if (current == GetCanNMRxInstance())
        {
            /* NM消息 - 调用NM模块处理 */
            CANNmProcessMessage((uint8_t *)CANRxBindData(current), current->config.rx_data_len);
            current = current->next;
            continue;
        }
        current = current->next;
...
void CANCommSendDataPackage_V2(void)
{
    ...
    while (current != NULL)
    {
        ...
        uint8_t package_total_len = data_len + sizeof(can_comm_protocol_head_t) + sizeof(can_comm_protocol_end_t);

        // 3. [优化] 预检查：如果 kfifo 没空间，根本不要开始打包，直接跳过
        // 这样避免了打包产生的 CPU 浪费
        if (kfifo_status(current->kfifo_ptr) == KFIFO_FULL ||
            (current->kfifo_ptr->size - kfifo_len(current->kfifo_ptr)) < package_total_len)
        {
        ...
        if (current->direct_tx_data_ptr != NULL && current->direct_tx_ready)
        {
            ...
            // 填充数据段
            __memcpy(temp_buffer + sizeof(can_comm_protocol_head_t), current->direct_tx_data_ptr, data_len);
            has_new_data = 1;
        }
        ...
        if (has_new_data)
        {
            ...
            uint8_t *p_crc = &temp_buffer[sizeof(can_comm_protocol_head_t) + data_len];
            *p_crc = get_CRC8_check_sum(&temp_buffer[sizeof(can_comm_protocol_head_t)], data_len, 0);
...
void CANCommSendFlush(void)
{
    ...
    while (current != NULL)
    {
        const uint8_t one_packet_lmax_len = floor_to_std(current->config.tx_data_len + protocol_len);
        /* 检查是否有待发送数据 */
        uint8_t get_fifo_len = one_packet_lmax_len < send_one_frame_len ? one_packet_lmax_len : send_one_frame_len;
        ...
        while (kfifo_len(current->kfifo_ptr) > 0)
        {
            if (can_comm_get_send_level(current->config.instance) > 0)
            {
                ...
                const uint8_t get_len = kfifo_get(current->kfifo_ptr, message.data, get_fifo_len);
        ...
        // 如果硬件 FIFO 满，不做任何事，保留 kfifo 数据下次发，或者清空 kfifo (根据实时性要求)
        // 建议：如果是电机控制指令，直接丢弃 kfifo 数据可能更好，保证发出去的是最新的
        current = current->next;
    }
}
...
#define CAN_BAUD_DATA 1000000.0f    // 5Mbps, 数据段 (如果是普通CAN，这里也填 1000000)

/* CAN FD 协议开销估算 (bits) */
/* 仲裁段开销: SOF(1)+ID(11)+RRS(1)+IDE(1)+FDF(1)+Res(1)+BRS(1)+ESI(1)+DLC(4) ≈ 22 bits */
#define CAN_FD_OVERHEAD_ARB 22

/* 数据段开销: CRC(17/21)+CRC_DEL(1)+ACK(1)+ACK_DEL(1)+EOF(7)+IFS(3) ≈ 30 bits (保守估计) */
...
uint8_t CANCommGetBusLoad(void)
{
    ...
    if (current_time > last_time && (current_time - last_time) >= 100)
    {
        ...
        // A. 仲裁段耗时 (运行在 1Mbps)
        // 每一帧都有固定的仲裁头
        float time_arb_us = (float)delta_total_frames * CAN_FD_OVERHEAD_ARB * (1000000.0f / CAN_BAUD_NOMINAL);

        // B. 数据段耗时 (运行在 5Mbps)
        // 包含：Payload数据(bytes*8) + 数据段的CRC/ACK等开销
        float bits_data_phase = (delta_total_bytes * 8) + (delta_total_frames * CAN_FD_OVERHEAD_DATA);
        float time_data_us = bits_data_phase * (1000000.0f / CAN_BAUD_DATA);

        // C. 总线占用总时间 (加上位填充系数)
        float total_bus_time_us = (time_arb_us + time_data_us) * CAN_STUFFING_FACTOR;
