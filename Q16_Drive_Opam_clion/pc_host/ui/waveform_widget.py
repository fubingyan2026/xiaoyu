...
from ui.styles_glass import PLOT_STYLE
...
class WaveformWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        # 创建波形容器
        container = QFrame()
        container.setObjectName("waveform_container")
        ...
        # 配置 pyqtgraph 样式
        pg.setConfigOptions(antialias=PLOT_STYLE.get('antialias', True))
        self.plot_widget.setBackground(PLOT_STYLE['background'])

        # 通用轴样式
        axis_pen = pg.mkPen(PLOT_STYLE['axis_color'], width=1)
        grid_alpha = PLOT_STYLE.get('grid_alpha', 0.5)

        # 电流图
        self.current_plot = self.plot_widget.addPlot(row=0, col=0)
        self.current_plot.setTitle('Current (A)', color='#00ff88')
        self.current_plot.setLabel('left', 'A', color='#00ff88')
        self.current_plot.showGrid(x=True, y=True, alpha=grid_alpha)
        self.current_plot.getAxis('left').setPen(axis_pen)
        self.current_plot.getAxis('bottom').setPen(axis_pen)

        self.id_curve = self.current_plot.plot(pen=pg.mkPen(PLOT_STYLE['colors'][0], width=PLOT_STYLE['line_width']))
        self.iq_curve = self.current_plot.plot(pen=pg.mkPen(PLOT_STYLE['colors'][1], width=PLOT_STYLE['line_width']))

        # 速度图
        self.velocity_plot = self.plot_widget.addPlot(row=1, col=0)
        self.velocity_plot.setTitle('Velocity (rad/s)', color='#ff9800')
        self.velocity_plot.setLabel('left', 'rad/s', color='#ff9800')
        self.velocity_plot.showGrid(x=True, y=True, alpha=grid_alpha)
        self.velocity_plot.getAxis('left').setPen(axis_pen)
        self.velocity_plot.getAxis('bottom').setPen(axis_pen)

        self.velocity_curve = self.velocity_plot.plot(pen=pg.mkPen(PLOT_STYLE['colors'][2], width=PLOT_STYLE['line_width']))

        # 位置图
        self.position_plot = self.plot_widget.addPlot(row=2, col=0)
        self.position_plot.setTitle('Position (rad)', color='#ff6b6b')
        self.position_plot.setLabel('left', 'rad', color='#ff6b6b')
        self.position_plot.setLabel('bottom', 'Time', color=PLOT_STYLE['foreground'])
        self.position_plot.showGrid(x=True, y=True, alpha=grid_alpha)
        self.position_plot.getAxis('left').setPen(axis_pen)
        self.position_plot.getAxis('bottom').setPen(axis_pen)

        self.position_curve = self.position_plot.plot(pen=pg.mkPen(PLOT_STYLE['colors'][3], width=PLOT_STYLE['line_width']))
        self.angle_curve = self.position_plot.plot(pen=pg.mkPen(PLOT_STYLE['colors'][4], width=PLOT_STYLE['line_width']))
