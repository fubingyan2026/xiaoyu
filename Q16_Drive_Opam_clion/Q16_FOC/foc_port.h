 ...
 *   - foc_port_config_t 结构体集中管理所有回调指针和硬件参数
 *   - foc_port_t 管理器封装初始化状态检查和回调分发逻辑
 ...
 * 采样时间等参数。通常在 foc_port_init 之后调用 foc_port_adc_init 触发。
 */
 ...
 *
 * 将占空比计数值写入硬件定时器比较寄存器。
 * 输入值已由 foc_port_pwm_update 转换为定时器计数值。
 *
...
typedef struct {
    /* ADC 相关 */
    foc_port_adc_read_cb_t adc_read;   /**< ADC 电流采样回调
                                            @note 必须注册，否则 foc_port_init 失败 */
    foc_port_adc_init_cb_t adc_init;   /**< ADC 初始化回调
                                            @note 必须注册 */
    /* PWM 相关 */
    foc_port_pwm_output_cb_t pwm_output; /**< PWM 输出回调
                                            @note 必须注册 */
    foc_port_pwm_start_cb_t pwm_start;   /**< PWM 启动回调
                                            @note 必须注册 */
    foc_port_pwm_stop_cb_t pwm_stop;     /**< PWM 停止回调
                                            @note 必须注册 */
    /* 编码器相关 */
    foc_port_encoder_read_cb_t encoder_read; /**< 编码器读取回调
                                                 @note 必须注册 */
    /* 系统相关 */
    foc_port_delay_cb_t delay_ms;       /**< 延时回调（可选） */
 ...
 * 每个 FOC 实例拥有独立的 port 对象，支持多电机控制。
 *
 * 使用方法：
 *   1. 填充 foc_port_config_t 配置
 *   2. 调用 foc_port_init() 初始化
 *   3. 调用 foc_port_adc_init() 初始化 ADC 硬件
 *   4. 在 FOC 电流环中调用 foc_port_adc_read()、foc_port_pwm_update() 等
 */
typedef struct {
    foc_port_config_t config;    /**< 硬件配置（含所有回调指针和硬件参数） */
    bool initialized;            /**< 初始化标志
                                    @note 由 foc_port_init() 设置，foc_port_deinit() 清除 */
} foc_port_t;
