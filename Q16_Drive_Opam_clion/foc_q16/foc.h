...
typedef struct {
    ...
    // 硬件抽象层配置
    foc_hal_config_t port_config; /**< 端口配置（含所有硬件回调函数指针）
                                        @note 移植新平台时主要修改此字段 */
...
struct foc_context {
    ...
    // 预计算的 Q16.16 常量（在 foc_init() 中一次性转换，避免运行时重复 FLOAT_TO_Q16_16）
    q16_16_t pwm_period_q; /**< FOC_PWM_PERIOD 的 Q16.16 格式（秒），用于 PLL dt */
    q16_16_t state_period_q; /**< STATE_PERIOD 的 Q16.16 格式（秒），用于 FSM 增量计算 */
    q16_16_t pll_kp_q; /**< PLL 比例增益（Q16.16 格式） */
    q16_16_t pll_ki_q; /**< PLL 积分增益（Q16.16 格式） */
    q16_16_t pll_speed_limit_q; /**< PLL 速度输出限制（Q16.16 格式，rad/s） */
