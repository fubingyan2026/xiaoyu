    ...
    def handle_response(self, response_str):
        ...
        if "23 03 02 02 00 00 00 36 CD" in response_str:
            if self.enroll_step == 0:
                # 第一次按压，开始第一次采集
                self.status_label.setText("正在采集（第1次），请抬起手指...")
                self.fingerprint_module.start_record(self.current_finger_id)
                self.enroll_step = 1
            elif self.enroll_step == 2:
                # 第二次按压，开始第二次采集
                self.status_label.setText("正在采集（第2次），请抬起手指...")
                self.progress_bar.setValue(1)
                self.fingerprint_module.continue_record()
                self.enroll_step = 3
            elif self.enroll_step == 4:
                # 第三次按压，开始第三次采集
                self.status_label.setText("正在采集（第3次），请抬起手指...")
        ...
        # 无手指或接触不好: EF 02 FF FF FF FF FF FF FF FF 00 00 23 03 02 02 00 04 00 F2 01
            ...
            if self.enroll_step == 1:
                # 第一次采集完成，等待第二次按压
                self.status_label.setText("请再次按压指纹（第2次）")
                self.enroll_step = 2
            elif self.enroll_step == 3:
                # 第二次采集完成，等待第三次按压
                self.status_label.setText("请再次按压指纹（第3次）")
                self.enroll_step = 4
            elif self.enroll_step == 5:
                # 第三次采集完成，保存指纹
                self.status_label.setText("正在保存指纹...")
