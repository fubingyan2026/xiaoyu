    ...
    const hal_adc_calibration_config_t* calibration_config)
{
    ...
    // 进入临界区，调用平台特定的校准函数
    HAL_ADC_ENTER_CRITICAL();
    hal_adc_error_t result = ctx->ops->calibrate(ctx, instance, calibration_config);
    HAL_ADC_EXIT_CRITICAL();
    ...
    void* user_data)
{
    ...
    hal_adc_error_t result = ctx->ops->register_callback(ctx, instance, callback, user_data);
    HAL_ADC_EXIT_CRITICAL();
    ...
    void* user_data)
{
    ...
    ctx->user_data = user_data;
    hal_adc_error_t result = ctx->ops->register_dma_callback(ctx, instance, callback, user_data);
    HAL_ADC_EXIT_CRITICAL();
