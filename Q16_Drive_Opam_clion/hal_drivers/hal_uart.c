    ...
    uint16_t* sent_size)
{
    ...
    // 进入临界区，调用平台特定的异步发送函数
    HAL_UART_ENTER_CRITICAL();
    hal_uart_error_t result = ctx->ops->send_async(ctx, instance, data, size, sent_size);
    HAL_UART_EXIT_CRITICAL();
    ...
    uint16_t* received_size)
{
    ...
    // 进入临界区，调用平台特定的异步接收函数
    HAL_UART_ENTER_CRITICAL();
    hal_uart_error_t result = ctx->ops->receive_async(ctx, instance, data, size, received_size);
    HAL_UART_EXIT_CRITICAL();
    ...
    uint8_t* data, uint32_t size)
{
    ...
    HAL_UART_ENTER_CRITICAL();
    hal_uart_error_t result = ctx->ops->receive_dma_to_idle(ctx, instance, data, size);
    HAL_UART_EXIT_CRITICAL();
