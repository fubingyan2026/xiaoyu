...
static hal_gpio_error_t stm32_gpio_init(hal_gpio_context_t* ctx,
    const hal_gpio_config_t* config)
{
    if (config == NULL || config->port >= PORT_MAP_SIZE || config->pin >= HAL_GPIO_PIN_MAX) {
    ...
    if (config->mode == HAL_GPIO_MODE_OUTPUT_PP || config->mode == HAL_GPIO_MODE_OUTPUT_OD) {
    ...
    hal_gpio_pin_state_t* state)
{
    ...
    (void)ctx;
    GPIO_PinState pin_state = HAL_GPIO_ReadPin(port_map[port], HAL_GPIO_PIN_MASK(pin));
