...
static bool stm32_uart_send(hal_uart_instance_e instance, const uint8_t* data, uint16_t size);
static bool stm32_uart_receive(hal_uart_instance_e instance, uint8_t* data, uint16_t size);
static uint16_t stm32_uart_send_async(hal_uart_instance_e instance, const uint8_t* data, uint16_t size);
static uint16_t stm32_uart_receive_async(hal_uart_instance_e instance, uint8_t* data, uint16_t size);
static bool stm32_uart_send_dma(hal_uart_instance_e instance, const uint8_t* data, uint32_t size);
static bool stm32_uart_receive_dma(hal_uart_instance_e instance, uint8_t* data, uint32_t size);
static bool stm32_uart_receive_dma_to_idle(hal_uart_instance_e instance, uint8_t* data, uint32_t size);
...
static hal_uart_dma_status_t stm32_uart_get_dma_status(hal_uart_instance_e instance);
static bool stm32_uart_set_baudrate(hal_uart_instance_e instance, hal_uart_baudrate_e baudrate);
static bool stm32_uart_set_rx_timeout(hal_uart_instance_e instance, uint32_t timeout);
...
static void configure_uart_dma(hal_uart_instance_e instance, const hal_uart_dma_config_t* dma_config);
...
{
    if (instance >= HAL_UART_INSTANCE_LEN)
        return false;
...
/**
  * @brief  配置UART DMA
  */
static void configure_uart_dma(const hal_uart_instance_e instance, const hal_uart_dma_config_t* dma_config)
{
    ...
    // 配置发送DMA
    if (dma_config->dma_mode == HAL_UART_DMA_TX || dma_config->dma_mode == HAL_UART_DMA_TX_RX)
    {
    ...
    // 配置接收DMA
    if (dma_config->dma_mode == HAL_UART_DMA_RX || dma_config->dma_mode == HAL_UART_DMA_TX_RX)
    {
...
static uint32_t convert_wordlength(const hal_uart_wordlength_e wordlength)
{
    switch (wordlength)
    {
    case HAL_UART_WORDLENGTH_8B: return UART_WORDLENGTH_8B;
    case HAL_UART_WORDLENGTH_9B: return UART_WORDLENGTH_9B;
...
static uint32_t convert_stopbits(hal_uart_stopbits_e stopbits)
{
    switch (stopbits)
    {
    case HAL_UART_STOPBITS_1: return UART_STOPBITS_1;
    case HAL_UART_STOPBITS_2: return UART_STOPBITS_2;
...
static uint32_t convert_hwcontrol(hal_uart_hwcontrol_e hwcontrol)
{
    switch (hwcontrol)
    {
    case HAL_UART_HWCONTROL_NONE: return UART_HWCONTROL_NONE;
    case HAL_UART_HWCONTROL_RTS: return UART_HWCONTROL_RTS;
    case HAL_UART_HWCONTROL_CTS: return UART_HWCONTROL_CTS;
    case HAL_UART_HWCONTROL_RTS_CTS: return UART_HWCONTROL_RTS_CTS;
...
static uint32_t convert_mode(const hal_uart_mode_e mode)
{
    switch (mode)
    {
    case HAL_UART_MODE_TX_RX: return UART_MODE_TX_RX;
    case HAL_UART_MODE_TX: return UART_MODE_TX;
    case HAL_UART_MODE_RX: return UART_MODE_RX;
...
static bool stm32_uart_send(const hal_uart_instance_e instance, const uint8_t* data, const uint16_t size)
{
    if (data == NULL || size == 0 || !validate_uart_instance(instance))
        return false;
    UART_HandleTypeDef* huart = uart_handle_map[instance];
    if (HAL_UART_Transmit(huart, (uint8_t*)data, size, huart->Init.BaudRate / 100) != HAL_OK)
        return false;
    return true;
}

static bool stm32_uart_receive(const hal_uart_instance_e instance, uint8_t* data, const uint16_t size)
{
    if (data == NULL || size == 0 || !validate_uart_instance(instance))
        return false;
    UART_HandleTypeDef* huart = uart_handle_map[instance];
...
static uint16_t stm32_uart_send_async(const hal_uart_instance_e instance, const uint8_t* data, const uint16_t size)
{
    if (data == NULL || size == 0 || !validate_uart_instance(instance))
        return 0;
    UART_HandleTypeDef* huart = uart_handle_map[instance];
...
static uint16_t stm32_uart_receive_async(const hal_uart_instance_e instance, uint8_t* data, const uint16_t size)
{
    if (data == NULL || size == 0 || !validate_uart_instance(instance))
        return 0;
    UART_HandleTypeDef* huart = uart_handle_map[instance];
...
static bool stm32_uart_send_dma(const hal_uart_instance_e instance, const uint8_t* data, const uint32_t size)
{
    if (data == NULL || size == 0 || !validate_uart_instance(instance))
        return false;
    UART_HandleTypeDef* huart = uart_handle_map[instance];
...
static bool stm32_uart_receive_dma(const hal_uart_instance_e instance, uint8_t* data, const uint32_t size)
{
    if (data == NULL || size == 0 || !validate_uart_instance(instance))
        return false;
    UART_HandleTypeDef* huart = uart_handle_map[instance];
...
static bool stm32_uart_receive_dma_to_idle(const hal_uart_instance_e instance, uint8_t* data, const uint32_t size)
{
    // 使用DMA接收，配合空闲中断检测帧结束
    if (!stm32_uart_receive_dma(instance, data, size))
        return false;
    // 启用空闲中断
...
static hal_uart_dma_status_t stm32_uart_get_dma_status(const hal_uart_instance_e instance)
{
    ...
    if (dma_status[instance].tx_busy)
    {
        ...
        if (huart->hdmatx != NULL)
        {
            dma_status[instance].tx_current_pos = dma_status[instance].tx_total_size -
    ...
    if (dma_status[instance].rx_busy)
    {
        ...
        if (huart->hdmarx != NULL)
        {
            dma_status[instance].rx_current_pos = dma_status[instance].rx_total_size -
...
static bool stm32_uart_set_baudrate(const hal_uart_instance_e instance, const hal_uart_baudrate_e baudrate)
{
    if (!validate_uart_instance(instance))
        return false;
    UART_HandleTypeDef* huart = uart_handle_map[instance];
...
static bool stm32_uart_set_rx_timeout(const hal_uart_instance_e instance, const uint32_t timeout)
{
    if (!validate_uart_instance(instance))
        return false;
...
void HAL_UART_IdleCallback(UART_HandleTypeDef* huart)
{
    ...
    for (int i = 0; i < HAL_UART_INSTANCE_LEN; i++)
    {
        if (uart_handle_map[i] == huart)
        {
            ...
            // 计算当前接收的数据长度
            const uint32_t received_size = dma_status[i].rx_total_size - __HAL_DMA_GET_COUNTER(huart->hdmarx);
            if (received_size > 0)
            {
                // 用户可以在这里处理接收到的数据
                // user_idle_callback(i, received_size);
                if (!dma_status[i].circular_mode) // 循环模式：自动继续接收，无需重新启动
                {
                    // 普通模式：重新启动接收
                    HAL_UART_Receive_DMA(huart, huart->pRxBuffPtr, dma_status[i].rx_total_size);
                }
            }
            break;
        }
