    ...
    hal_uart_dma_status_t* status)
{
    ...
    if (dma_status[instance].tx_busy) {
        if (huart->hdmatx != NULL) {
            dma_status[instance].tx_current_pos = dma_status[instance].tx_total_size - __HAL_DMA_GET_COUNTER(huart->hdmatx);
        }
    }

    if (dma_status[instance].rx_busy) {
        if (huart->hdmarx != NULL) {
            dma_status[instance].rx_current_pos = dma_status[instance].rx_total_size - __HAL_DMA_GET_COUNTER(huart->hdmarx);
        }
    }
