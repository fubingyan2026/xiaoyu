    ...
    const hal_adc_config_t* config)
{
    ...
    hadc->Init.ScanConvMode = (config->scan_mode == HAL_ADC_SCAN_MODE_ENABLE) ? ENABLE : DISABLE;
    hadc->Init.ContinuousConvMode = (config->continuous == HAL_ADC_CONTINUOUS_ENABLE) ? ENABLE : DISABLE;
    hadc->Init.DiscontinuousConvMode = DISABLE;
    hadc->Init.NbrOfDiscConversion = 0;
    hadc->Init.ExternalTrigConv = ADC_SOFTWARE_START;
    hadc->Init.ExternalTrigConvEdge = ADC_EXTERNALTRIGCONVEDGE_NONE;
    hadc->Init.NbrOfConversion = config->nbr_of_conversion;
    hadc->Init.DMAContinuousRequests = (config->dma_mode == HAL_ADC_DMA_ENABLE) ? ENABLE : DISABLE;
    ...
    hal_adc_context_t* ctx, const hal_adc_channel_config_t* channel_config)
{
    ...
    sConfig.SamplingTime = stm32_adc_get_sampletime_hal(channel_config->sample_time);
