...
static const char* s_key_event_names[KEY_BASE_EVENT_MAX] = KEY_BASE_EVENT_NAME_TABLE;
 ...
 */
static void on_led_gpio_edge_change(led_handle_t* instance, bool rising,
    void* user_data)
{
    (void)instance;
    (void)rising;
...
{
    hal_gpio_write(&gpio_ctx, HAL_GPIO_PORT_B, HAL_GPIO_PIN_9,
        on ? HAL_GPIO_PIN_RESET : HAL_GPIO_PIN_SET);
}

static bool led_read_pin(void)
{
    hal_gpio_pin_state_t st;
    hal_gpio_read(&gpio_ctx, HAL_GPIO_PORT_B, HAL_GPIO_PIN_9, &st);
    return st == HAL_GPIO_PIN_RESET;
}
...
void key_func_init(void)
{
    ...
    /* 初始化 LED GPIO */
    hal_gpio_config_t led_gpio_cfg = {
    ...
    hal_gpio_init(&gpio_ctx, &led_gpio_cfg);
    ...
    /* 注册 LED 回调 */
    led_set_callbacks(s_led_instance, on_led_state_change,
        on_led_blink_phase_change, on_led_gpio_edge_change, NULL);
