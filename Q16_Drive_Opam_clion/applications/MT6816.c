...
static uint16_t RINE_MT6816_SPI_Get_AngleData(void)
{
    ...
    {
        MT6816_SPI_CS_L();
        HAL_SPI_TransmitReceive(&MT6816_SPI_Get_HSPI, (uint8_t *)&data_t[0], (uint8_t *)&data_r[0], 1, 10);
        MT6816_SPI_CS_H();

        MT6816_SPI_CS_L();
        HAL_SPI_TransmitReceive(&MT6816_SPI_Get_HSPI, (uint8_t *)&data_t[1], (uint8_t *)&data_r[1], 1, 10);
        MT6816_SPI_CS_H();
...
Retry:
    ...
    const HAL_StatusTypeDef spiStatus = HAL_SPI_TransmitReceive(
        &MT6816_SPI_Get_HSPI, (uint8_t *)&txData, (uint8_t *)MT6701_data_buffer, sizeof(MT6701_data_buffer), 100);
    if (spiStatus != HAL_OK)
    {
        MT6816_SPI_CS_H();
    ...
    mt6701_RAW.Status.magnetic_state = ((MT6701_data_buffer[1] >> 14) | (MT6701_data_buffer[0] & 0x03) >> 2) & 0x03;
    mt6701_RAW.Status.Push_button_state = ((MT6701_data_buffer[1] >> 14) | (MT6701_data_buffer[0] & 0x03) >> 1) & 0x01;
    mt6701_RAW.Status.Loss_of_Track = ((MT6701_data_buffer[1] >> 14) | (MT6701_data_buffer[0] & 0x03) >> 0) & 0x01;
...
static uint8_t crc6_itu(const uint16_t *data, uint32_t length)
{
    ...
    const uint8_t crc_data[3] = {data[0] >> 10 & 0x3F, data[0] >> 4 & 0x3F,
