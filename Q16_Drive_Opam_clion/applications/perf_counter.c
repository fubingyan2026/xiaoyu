...
static volatile int64_t s_lOldTimestamp;
volatile int32_t g_nOffset = 0;
static volatile int32_t s_nUSUnit = 1;
static volatile int32_t s_nMSUnit = 1;
static volatile int32_t s_nMSResidule = 0;
static volatile int32_t s_nUSResidule = 0;
static volatile int32_t s_nSystemMS = 0;
static volatile int32_t s_nSystemUS = 0;

static volatile int64_t s_lSystemClockCounts = 0;
...
//        set Priority for Systick Interrupt */ SysTick->VAL   = 0UL; /* Load
//        the SysTick Counter Value */ SysTick->CTRL  =
...
__STATIC_INLINE int32_t check_systick(void)
{
     ...
     *        - Condition 1: when assigning nTemp with the register value
     * (LOAD-VAL), the underflow didn't happen but when we check the PENDSTSET
     * bit, the underflow happens, for this condition, we should not do any
     * compensation. When this happens, the (LOAD-nTemp) is smaller than
     * PERF_CNT_COMPENSATION_THRESHOLD (a small value) as long as LOAD is bigger
     * than (or equals to) the PERF_CNT_COMPENSATION_THRESHOLD;
     *        - Condition 2: when assigning nTemp with the register value
     * (LOAD-VAL), the VAL is zero and underflow happened and the PENDSTSET bit is
     * set, for this condition, we should not do any compensation. When this
     * happens, the (LOAD-nTemp) is equals to zero.
     *        - Condition 3: when assigning nTemp with the register value
     * (LOAD-VAL), the underflow has already happened, hence the PENDSTSET is set,
     * for this condition, we should compensate the return value. When this
     * happens, the (LOAD-nTemp) is bigger than (or equals to)
     * PERF_CNT_COMPENSATION_THRESHOLD. The following code implements an
     * equivalent logic.
     */
...
#endif
__attribute__((noinline)) int64_t clock(void)
{
...
uint32_t EventRecorderTimerGetFreq(void) { return SystemCoreClock; }

/// Get timer count.
/// \return       timer count (32-bit)
uint32_t EventRecorderTimerGetCount(void) { return get_system_ticks(); }

__WEAK
task_cycle_info_t* get_rtos_task_cycle_info(void) { return NULL; }

void init_task_cycle_counter(void)
{
    struct __task_cycle_info_t* ptRootAgent = (struct __task_cycle_info_t*)get_rtos_task_cycle_info();
    if (NULL == ptRootAgent) {
        return;
    }
...
bool perfc_check_task_stack_canary_safe(void)
{
    ...
    do {
        ...
        if ((MAGIC_WORD_CANARY == ptRootAgent->wMagicWord) || (MAGIC_WORD_AGENT_LIST_VALID == ptRootAgent->wMagicWord)) {
            return true;
        }
    } while (0);
...
__stop_task_cycle_counter(task_cycle_info_t* ptInfo)
{
    struct __task_cycle_info_t* ptRootAgent = (struct __task_cycle_info_t*)get_rtos_task_cycle_info();
    if (NULL == ptRootAgent) {
        return 0;
