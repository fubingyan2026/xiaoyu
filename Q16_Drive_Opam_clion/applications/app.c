...
{
    // BSP_Printf("Received INS:%d\r\n", (int) (can_rx_ins.f_angle[1]*RAD_TO_ANGLE));
    //    BSP_Printf("HALL_ANGLE:%d,%d\r\n", (int)(gFocMachine.pll_phase * RAD_TO_ANGLE), (int)(gFocMachine.alginIq *
    //    1000));
...
void AppInit(void)
{
    ...
    tk_timer_start(timer_driverTask, TIMER_MODE_LOOP, (uint32_t)(STATE_PERIOD * 1000));
...
void AppRunning(void)
{
    ...
    if (true == tk_event_recv(can_send_event, TIM_EVENT_FLAG, TK_EVENT_OPTION_OR | TK_EVENT_OPTION_CLEAR, NULL))
    {
        /* NM状态机处理 */
        // can_nm_task();
        // CANNmStateMachine();
