...
uint16_t s7_get_frame_len(uint8_t* buffer, uint16_t len) {
  ...
  /* S7帧格式: head(2) + proto_id(1) + msg_type(1) + func(1) + data(n) + crc(2)
   * + tail(1) */
...
void uart_it_init(void) {
  ...
  if (fifo_usart1_rx != NULL) {
    (void)hal_uart_receive_dma(&uart1_ctx, HAL_UART_INSTANCE_1,
                               fifo_usart1_rx->buffer, fifo_usart1_rx->size);
  }
