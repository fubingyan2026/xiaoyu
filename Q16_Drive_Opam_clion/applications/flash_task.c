...
int flash_task_init(void)
{
    ...
    /* 注册发布者 */
    message_center_error_t err = message_center_publisher_register(&g_flash_task_publisher, config);
 ...
 * @brief 获取任务管理器实例
 */
flash_task_mgr_t* flash_task_get_instance(void) { return &g_flash_task_mgr; }
