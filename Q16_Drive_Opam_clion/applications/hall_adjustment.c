...
static void HALL_Adjust_EXE_Flow(hall_adjust_t *_hall_adjust)
{
    ...
    case ADJUST_ROTATION:
        ...
        if (_hall_adjust->motorTargetElectricalAngle < (float)hall_adjust.motor_poles * M_2PI * rotation_circle * 1.25f)
        {
    ...
    case ADJUST_PROCESS:
        ...
        {
            hall_save_param.adcAmplitudeBias[i] = (int16_t)((adc_max_value[i] + adc_min_value[i]) * 0.5f);
            hall_save_param.adcAmplitudeMax[i] = (int16_t)((adc_max_value[i] - adc_min_value[i]) * 0.5f);
        }

        flash_task_request(FLASH_TASK_WRITE_HALL, &hall_save_param, sizeof(hall_save_param));
...
static void HALL_Adjust_Init(void)
{
    for (uint8_t i = 0; i < 2; i++)
    {
        hall_filter_init(&hall_filter_normal[i], hall_filter_fcut_Normal, FOC_PWM_PERIOD);
    }
    for (uint8_t i = 0; i < 2; i++)
    {
...
static void hall_filter_init(pt1Filter_t *filter, const uint16_t f_cut, const float dt)
{
...
static void HALL_Adjust_Task(void)
{
    HALL_Adjust_EXE_Flow(&hall_adjust);
}
...
static uint16_t HALL_Adjust_Get_Angle(void)
{
    for (uint8_t i = 0; i < ADC_CH_NUM; i++)
    {
        pt1FilterApply(&hall_filter_normal[i], (float)hall_adjust.adcOriginalValue[i]);
    }

    float _y = (float)((int32_t)hall_filter_normal[1].state - hall_save_param.adcAmplitudeBias[1]) /
               (float)hall_save_param.adcAmplitudeMax[1];
    float _x = (float)((int32_t)hall_filter_normal[0].state - hall_save_param.adcAmplitudeBias[0]) /
