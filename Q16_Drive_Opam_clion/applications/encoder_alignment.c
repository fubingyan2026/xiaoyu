 ...
 */
static bool validate_sector_deltas(const uint16_t* angle_buffer,
 ...
 * @return 基于统计分析检测到的方向
 */
static motor_direction_t calculate_direction_average(
    const uint16_t* angle_buffer, const encoder_calibration_t* calib);
 ...
 */
static bool find_current_sector(uint16_t angle,
 ...
 */
static float calculate_sector_angle(uint16_t angle,
                                    const encoder_calibration_t* calib);
 ...
 */
static inline bool encoder_is_calibration_valid(
    const encoder_calibration_t* calib) {
...
bool encoder_alignment_init(void) {
  ...
  const size_t bytes_read =
      ef_get_env_blob(FLASH_MAGIC_ENCODER, &g_motor_flash_cfg,
                      sizeof(motor_flash_config_t), NULL);
  ...
  // 验证并处理校准数据
  if (!encoder_detect_direction(g_motor_flash_cfg.angle_map,
                                &g_encoder_calib)) {
    return false;
  }
...
float encoder_track_sector(uint16_t raw_angle, encoder_calibration_t* calib) {
  ...
  if (calib->direction == MOTOR_DIR_FORWARD) {
    ...
    corrected_angle =
        (calib->encoder_lines - 1 - raw_angle) & (calib->encoder_lines - 1);
  }
  ...
  calib->electrical_angle =
      (sector_fraction + sector_offset) * ELEC_ANGLE_FACTOR;
...
static motor_direction_t calculate_direction_average(
  ...
  const int16_t test_idx =
      (calib->zero_offset > half_steps) ? (half_steps - 2) : (half_steps + 1);
  const int32_t fallback_delta =
      (int32_t)angle_buffer[test_idx + 1] - (int32_t)angle_buffer[test_idx];
...
static float calculate_sector_angle(uint16_t angle,
  ...
  if (sector_end > sector_start) {
    ...
    if (angle >= wraparound_threshold) {
      ...
      // 角度环绕到开头
      angle_in_sector = (int32_t)calib->encoder_lines - (int32_t)sector_start +
                        (int32_t)angle;
    }
  }
