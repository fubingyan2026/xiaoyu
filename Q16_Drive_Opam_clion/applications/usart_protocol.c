    ...
    const uint8_t* data, uint16_t len)
{
    ...
    uint16_t frame_len = s7_build_frame(msg_type, func_code, data, len, ctx->output_buffer);
    ...
    uint16_t* resp_len)
{
    ...
    *resp_len = 0U;
    ...
    if (len >= 3U) {
        s_current_ctx->stream_cfg.interval_ms = (uint16_t)((uint16_t)data[1U] | ((uint16_t)data[2U] << 8U));
    } else if (len >= 2U) {
...
void usart_protocol_parse_task(usart_protocol_context_t* ctx)
{
    ...
    const protocol_parser_error_t err = protocol_parser_parse(&ctx->parser_ctx, &frame_len, &frame_data);
...
static protocol_parser_error_t s7_check_cb(uint8_t* buffer, uint16_t len)
{
    ...
    uint16_t recv = (uint16_t)buffer[len - 3U] | ((uint16_t)buffer[len - 2U] << 8U);
