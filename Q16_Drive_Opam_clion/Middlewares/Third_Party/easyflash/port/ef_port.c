/**
 * @file    ef_port.c
 * @brief   EasyFlash Flash操作接口 - STM32G4系列移植层
 * @author  FOC Development Team
 * @date    2026-02-04
 *
 * @note    基于Armink EasyFlash库移植，支持STM32G431CBT6
 */
...
EfErrCode ef_port_erase(const uint32_t addr, const size_t size) {
  ...
  for (uint16_t i = 0; i < erase_pages; i++) {
    const HAL_StatusTypeDef flash_status =
        HAL_FLASHEx_Erase(&erase_init, &error);

    if (flash_status != HAL_OK) {
      /* 擦除失败，记录错误并退出 */
      DEBUG_LOGE("flash 擦除错误", "flash_status: %d, error: %d, 地址: 0x%08X",
                 flash_status, error, addr);
...
EfErrCode ef_port_write(uint32_t offset, const uint32_t* buf, size_t size) {
  ...
  for (size_t i = 0; (i < size); i += FLASH_PROGRAM_SIZE) {
    /* 复制8字节数据到对齐缓冲区 */
    memcpy(&write_data, data, FLASH_PROGRAM_SIZE);
    ...
    if ((read_data != *(volatile uint64_t*)data) || (flash_wstatus != HAL_OK)) {
      /* 写入错误，记录详细错误信息 */
      DEBUG_LOGE("flash 写入错误", "flash_status: %d, 地址=0x%08X", flash_wstatus,
                 addr);
      result = EF_WRITE_ERR;
...
void ef_log_debug(const char* file, const long line, const char* format, ...) {
  ...
  va_end(args);
  DEBUG_LOGD("easyflash_debug", "(%s:%ld) %s", file, line, msg_buf);
}

static inline void ef_log_helper(debug_level_t level, const char* tag,
                                 const char* format, va_list args) {
  char msg_buf[256];
  vsnprintf(msg_buf, sizeof(msg_buf), format, args);
  DEBUG_LOGI(tag, "%s", msg_buf);
}

void ef_log_info(const char* format, ...) {
  va_list args;
  va_start(args, format);
  ef_log_helper(DEBUG_LEVEL_INFO, "easyflash_info", format, args);
  va_end(args);
}
...
void ef_print(const char* format, ...) {
  va_list args;
  va_start(args, format);
  ef_log_helper(DEBUG_LEVEL_INFO, "easyflash_print", format, args);
  va_end(args);
}
