/**
 * @brief: add your brief!
 * @FilePath: tk_queue.c
 * @author: fubingyan qq:3245784484
 * @Date: 2025-10-20 12:14:56
 * @LastEditTime: 2025-12-05 21:23:08
 * @version: V1.0.0
 *  * Date             Notes
 * 2019-12-12          the first version
 * 2020-01-14          add keep fresh mode
 * 2020-01-29          add assert for developer
 * 2020-02-28          add value type define
 * 2020-03-04          add queue clean code
 * 2020-06-04          support any type
 * 2020-11-28          add queue peep&remove code
 * @note: add your note!
                                 ...
                                 bool keep_fresh)
{
    ...
    if ((queue = __malloc(sizeof(struct tk_queue))) == NULL)
        return NULL;
    queue->queue_size = queue_size;
    queue->max_queues = max_queues;
    queue->queue_pool = __malloc(queue->queue_size * queue->max_queues);
...
bool tk_queue_push(struct tk_queue* queue, void* pval)
{
    ...
    if (tk_queue_full(queue))
    {
        ...
        {
            __memcpy((uint8_t*)queue->queue_pool + (queue->rear * queue->queue_size),
                     (uint8_t*)pval, queue->queue_size);
