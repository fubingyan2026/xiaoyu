/**
 * @file    angle_sensor_mt6701.c
 * @author  fubingyan
 * @version V2.0.0
 * @date    2026-05-22
 * @brief   MT6701 磁编码器传感器实现（SPI）
 * @attention
 *
...
static int sensor_mt6701_calibrate(angle_sensor_context_t* ctx);
static bool sensor_mt6701_is_calibrated(angle_sensor_context_t* ctx);
...
static float sensor_mt6701_get_velocity_rads(angle_sensor_context_t* ctx);
static void sensor_mt6701_update(angle_sensor_context_t* ctx);
static void sensor_mt6701_get_info(angle_sensor_context_t* ctx,
    angle_sensor_info_t* info);
static void sensor_mt6701_set_offset(angle_sensor_context_t* ctx, float offset);

/* Private variables ---------------------------------------------------------*/

static const angle_sensor_ops_t sensor_ops_mt6701 = {
