...
#define BIT_1 76 // 1比较值为61-->850us
#define BIT_0 30 // 0比较值为28-->400us

#define PIXEL_SIZE 1
#define WS28xx_PWM_hTIMER htim3
#define WS28xx_PWM_Chnnel TIM_CHANNEL_2

#define HSV_H_MAX 360
#define HSV_H_MIN 1

#define HSV_S_MAX 1
#define HSV_S_MIN 0.6f // 0.6保证能显示基本颜色（而不会变白色失真）

#define HSV_V_MAX 1
#define HSV_V_MIN 0.005f
// 整个WS28xx_DataTypeStruct结构体将被以PWM方式发送
typedef struct
{
	// 1个0等待PWM稳定
	unsigned short ColorStartData[6];
	// 实际GRB数据(已经转换为PWM对应的值)
	unsigned short ColorRealData[24 * PIXEL_SIZE];
	// 1结束位为0
	unsigned short ColorEndData[6];
...
#define MY_LIMIT(x, max, min) ((x < max) ? ((x > min) ? x : min) : max)
