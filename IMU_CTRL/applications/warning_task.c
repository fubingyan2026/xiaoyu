...
{
    static uint8_t error_code, last_error_code;
    static uint8_t priority;
    error_code = 0;
    priority = 0x07; // 初始化为最低优先级，为后续查找更高优先级
    // find error
    // 发现错误
    ...
    while (this->next_daemon_lists)
    {
        ...
        {
            if (priority > this->config.warn_priority)
            {
                priority = this->config.warn_priority;
                error_code = this->config.warn_error_code;
            }
        }
    }
    ...
    reset_code:
        if (last_error_code)
        {
            LedSetState(led_instance, LED_STATE_BLINK_CODE);
            LedSetBlinkInterval(led_instance, 120, 500, error_code);
        }
        else // 没有错误.
        {
            LedSetState(led_instance, LED_STATE_BLINK_CODE);
            LedSetBlinkInterval(led_instance, 500, 1000, can_save_id);
        }
    }
}
