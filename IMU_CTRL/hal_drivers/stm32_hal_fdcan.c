...
static bool stm32_fdcan_init(const hal_fdcan_config_t* config);
static bool stm32_fdcan_send(hal_fdcan_instance_e instance, const hal_fdcan_message_t* message);
static bool stm32_fdcan_receive(hal_fdcan_instance_e instance, hal_fdcan_message_t* message);
static uint32_t stm32_fdcan_receive_level(hal_fdcan_instance_e instance, uint8_t fifo_address);
static uint32_t stm32_fdcan_get_send_level(hal_fdcan_instance_e instance);

static bool stm32_fdcan_set_filter(hal_fdcan_instance_e instance, const hal_fdcan_filter_t* filter);
static bool stm32_fdcan_set_mode(hal_fdcan_instance_e instance, hal_fdcan_mode_e mode);
static uint32_t stm32_fdcan_get_error_count(hal_fdcan_instance_e instance);
static void stm32_fdcan_irq_handler(hal_fdcan_instance_e instance);
static uint8_t round_up_special(uint8_t x);
static uint8_t get_can_rx_length(uint32_t rx_DL);
static bool validate_fdcan_instance(hal_fdcan_instance_e instance);
static void enable_fdcan_clock(hal_fdcan_instance_e instance);
static void configure_fdcan_interrupts(hal_fdcan_instance_e instance);
...
/**
  * @brief  配置FDCAN中断
  */
static void configure_fdcan_interrupts(const hal_fdcan_instance_e instance)
{
    switch (instance)
    {
    case HAL_FDCAN_INSTANCE_1:
...
void platform_fdcan_init(void)
{
    /* 设置FDCAN操作函数 */
    hal_fdcan_set_ops(&stm32_fdcan_ops);
}
...
static bool stm32_fdcan_init(const hal_fdcan_config_t* config)
{
    ...
    case HAL_FDCAN_MODE_SILENT_LOOPBACK:
        hfdcan->Init.Mode = FDCAN_MODE_INTERNAL_LOOPBACK | FDCAN_MODE_BUS_MONITORING;
        break;
    default:
        hfdcan->Init.Mode = FDCAN_MODE_NORMAL;
        break;
    ...
    for (uint8_t i = 0; i < config->filter_count; i++)
    {
        FDCAN_FilterTypeDef bsp_filter = {
            .IdType = config->filters[i].is_extended ? FDCAN_EXTENDED_ID : FDCAN_STANDARD_ID,
...
static bool stm32_fdcan_send(const hal_fdcan_instance_e instance, const hal_fdcan_message_t* message)
{
    ...
    switch (tx_data_len)
    {
    case 12: len_code = FDCAN_DLC_BYTES_12;
        break;
    case 16: len_code = FDCAN_DLC_BYTES_16;
        break;
    case 20: len_code = FDCAN_DLC_BYTES_20;
        break;
    case 24: len_code = FDCAN_DLC_BYTES_24;
        break;
    case 32: len_code = FDCAN_DLC_BYTES_32;
        break;
    case 48: len_code = FDCAN_DLC_BYTES_48;
        break;
    case 64: len_code = FDCAN_DLC_BYTES_64;
        break;
    default: len_code = tx_data_len;
        break;
    }

    FDCAN_TxHeaderTypeDef tx_header;
    tx_header.Identifier = message->id;
    tx_header.IdType = message->is_extended ? FDCAN_EXTENDED_ID : FDCAN_STANDARD_ID;
    tx_header.TxFrameType = message->is_remote ? FDCAN_REMOTE_FRAME : FDCAN_DATA_FRAME;
    ...
    if (HAL_FDCAN_AddMessageToTxFifoQ(hfdcan, &tx_header, message->data) != HAL_OK)
    {
        return false;
    }
...
static uint8_t get_can_rx_length(const uint32_t rx_DL)
{
    ...
    default: return (uint8_t)rx_DL;
    }
}

static bool stm32_fdcan_receive(const hal_fdcan_instance_e instance, hal_fdcan_message_t* message)
{
    if (message == NULL || !validate_fdcan_instance(instance))
    {
        return false;
...
static uint32_t stm32_fdcan_receive_level(hal_fdcan_instance_e instance, uint8_t fifo_address)
{
    ...
    if (fifo_address)
    {
        return  HAL_FDCAN_GetRxFifoFillLevel(hfdcan, FDCAN_RX_FIFO1) ;// FIFO不为空,有可能在其他中断时有多帧数据进入
    }
    return  HAL_FDCAN_GetRxFifoFillLevel(hfdcan, FDCAN_RX_FIFO0) ;// FIFO不为空,有可能在其他中断时有多帧数据进入
}
...
static bool stm32_fdcan_set_filter(const hal_fdcan_instance_e instance, const hal_fdcan_filter_t* filter)
{
...
static bool stm32_fdcan_set_mode(const hal_fdcan_instance_e instance, hal_fdcan_mode_e mode)
{
...
static uint32_t stm32_fdcan_get_error_count(const hal_fdcan_instance_e instance)
{
    if (!validate_fdcan_instance(instance))
    {
        return 0;
    ...
    return error_counters.RxErrorCnt + error_counters.TxErrorCnt;
}

static void stm32_fdcan_irq_handler(const hal_fdcan_instance_e instance)
{
    if (!validate_fdcan_instance(instance))
    {
        return;
...
{
    hal_fdcan_irq_handler(HAL_FDCAN_INSTANCE_2);
}
...
{
    hal_fdcan_irq_handler(HAL_FDCAN_INSTANCE_3);
}
