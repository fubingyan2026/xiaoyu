 ...
 * @version: V1.0.0
 *           V1.0.1 2025-08-17 23:58:01 更新了头和尾的限幅值，防止内存随即初始化带来的的计算耗时
 *           V1.0.2 2025-09-05 19:38:06 修改了Create_FIFO_Buffer函数，传入外部申请的内存
 * @note: add your note!
...
/* 加锁 */
static void lock(kfifo_t *fifo)
{
    // __disable_irq();
}

/* 解锁 */
static void unlock(kfifo_t *fifo)
{
    // __enable_irq();
}
 ...
 */
kfifo_t *kfifo_init(unsigned char *buffer, const unsigned int size, int gfp_mask, uint32_t *lock)
{
    /* 检查size是否为2的幂 */
 ...
 * @return 实际放入的数据长度
 */
unsigned int __kfifo_put(kfifo_t *fifo, const unsigned char *buffer, unsigned int len)
{
    /* 计算可放入的数据长度 */
    len = min(len, fifo->size - fifo->in + fifo->out);
    /* 计算第一次拷贝的长度 */
    const unsigned int l = min(len, fifo->size - (fifo->in & (fifo->size - 1)));
    __memcpy(fifo->buffer + (fifo->in & (fifo->size - 1)), (void *)buffer, l);
    /* 计算第二次拷贝的长度 */
    __memcpy(fifo->buffer, (void *)(buffer + l), len - l);
    /* 更新in指针 */
    fifo->in += len;
    return len;
}
 ...
 * @return 实际取出的数据长度
 */
unsigned int __kfifo_get(kfifo_t *fifo, unsigned char *buffer, unsigned int len)
{
    /* 计算可取出的数据长度 */
    len = min(len, fifo->in - fifo->out);
    /* 计算第一次拷贝的长度 */
    const unsigned int l = min(len, fifo->size - (fifo->out & (fifo->size - 1)));
    __memcpy(buffer, fifo->buffer + (fifo->out & (fifo->size - 1)), l);
    /* 计算第二次拷贝的长度 */
    __memcpy(buffer + l, fifo->buffer, len - l);
    /* 更新out指针 */
    fifo->out += len;
    return len;
}
...
uint32_t kfifo_peek(const kfifo_t *fifo, uint8_t *buffer, uint32_t len, uint32_t offset)
{
    ...
    __memcpy(buffer, fifo->buffer + (out & (fifo->size - 1)), l);
    /* 然后查看剩余的数据（如果有回绕） */
    __memcpy(buffer + l, fifo->buffer, len - l);
    return len;
}
 ...
 * @return 数据长度
 */
static unsigned int __kfifo_len(kfifo_t *fifo)
{
...
/**重置kfifo（无锁版本） */
static void __kfifo_reset(kfifo_t *fifo)
{
    fifo->in = fifo->out = 0;
}
 ...
 * @param len 要清除的数据长度
 * @return 实际清除的数据长度，如果请求的长度大于当前缓冲区中的数据长度，则返回当前缓冲区中的数据长度
 */
...
void kfifo_move_in(kfifo_t *fifo, const unsigned int len)
{
    ...
    const unsigned int move_in_len = len >= current_in ? len - current_in : len + fifo->size - current_in;
