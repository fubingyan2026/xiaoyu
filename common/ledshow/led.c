...
{
    hal_gpio_pin_state_e state = on ? handle->config.active_level : (hal_gpio_pin_state_e)!handle->config.active_level;
    hal_gpio_pin_state_e old_state = hal_gpio_read(handle->config.port, handle->config.pin);
    ...
    if (handle->gpio_edge_cb && old_state != state)
    {
        ...
        {
            ((led_gpio_edge_callback_t)handle->gpio_edge_cb)(handle, state, handle->callback_user_data);
        }
    }
}
...
{
    hal_gpio_pin_state_e state = hal_gpio_read(handle->config.port, handle->config.pin);
...
static fsm_state_t led_fsm_blink_handler(fsm_context_t *ctx)
{
    ...
    {
        if (led_time_diff(now, handle->last_toggle_time) >= handle->config.blink_interval_ms)
        {
    ...
    else
    {
        ...
        if (led_time_diff(now, handle->interval_start_time) >= handle->config.blink_interval_wait_ms)
        {
            /* 否则是连续闪烁模式，继续闪烁 */
...
{
    if (!name || !led_system_initialized)
        return NULL;
    led_handle_t *curr = led_master;
    while (curr)
    {
...
{
    if (!config || !instance || !config->led_name)
        return LED_ERR_INVALID_PARAM;
    if (!led_system_initialized)
        return LED_ERR_INTERNAL;
...
led_handle_t *LedRegister(const led_config_t *config)
{
    ...
    led_error_e err = LedRegisterStatic(config, instance);
    if (LED_IS_ERR(err))
    {
        __free(instance);
        return NULL;
...
void LedSetState(led_handle_t *instance, led_state_t state)
{
    ...
    kfifo_put((kfifo_t *)instance->cmd_fifo, (unsigned char *)&cmd, sizeof(led_cmd_t));
}
...
led_error_e LedSetBlinkInterval(led_handle_t *instance, uint16_t interval_ms, uint16_t interval_wait_ms, uint16_t counts)
{
    ...
    /* 检查参数是否真的变化 */
    bool param_changed = (instance->config.blink_interval_ms != interval_ms) ||
    ...
    if (fsm_get_current_state(&instance->fsm) == LED_STATE_BLINK_CODE)
    {
        ...
        if (is_led_on)
        {
            ...
            led_cmd_t cmd = {
                .target_state = LED_STATE_BLINK_CODE,
                .blink_interval_ms = interval_ms,
                .blink_interval_wait_ms = interval_wait_ms,
                .blink_counts = counts};
            return kfifo_put((kfifo_t *)instance->cmd_fifo, (unsigned char *)&cmd, sizeof(led_cmd_t)) == sizeof(led_cmd_t)
                       ? LED_OK
                       : LED_ERR_INTERNAL;
        }
        else
...
{
    if (!instance)
        return BLINK_CODE_PHASE_INTERVAL;
...
static void process_led_cmds(led_handle_t *handle)
{
    ...
    while (kfifo_get(handle->cmd_fifo, (unsigned char *)&cmd, sizeof(led_cmd_t)) == sizeof(led_cmd_t))
    {
        ...
        if (cmd.target_state == LED_STATE_BLINK_CODE)
        {
            ...
            if (cmd.blink_counts > 0)
                handle->config.blink_counts = cmd.blink_counts;
...
static void check_blink_phase_change(led_handle_t *handle)
{
    ...
    {
        /* 检测到阶段变化，触发回调 */
        ((led_blink_phase_callback_t)handle->blink_phase_cb)(handle, handle->blink_code_phase, handle->callback_user_data);
        handle->last_blink_code_phase = handle->blink_code_phase;
    }
}
...
{
    if (!led_system_initialized || !led_master)
        return;
