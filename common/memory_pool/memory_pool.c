...
__attribute__((aligned(LWMEM_CFG_ALIGN_NUM))) static uint8_t region1_data[1024 * 6];
#elif defined(__IS_COMPILER_ARM_COMPILER_6__)
#define __INT_TO_STR(x) #x
#define INT_TO_STR(x) __INT_TO_STR(x)
__attribute__((aligned(LWMEM_CFG_ALIGN_NUM))) static uint8_t region1_data[1024 * 6];
#else
__attribute__((section(".ram"), aligned(LWMEM_CFG_ALIGN_NUM))) static uint8_t region1_data[1024 * 6];
#endif
...
void *memmove_optimized(void *dst, const void *src, size_t count)
{
    ...
    // 检查内存重叠
    if ((uintptr_t)dst > (uintptr_t)src && (uintptr_t)dst < (uintptr_t)src + count)
    {
        // 重叠情况：从后向前复制
