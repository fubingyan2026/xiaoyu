 ...
 * @LastEditTime: 2025-10-20 20:01:42
 * @version: V1.0.0
 * Change Logs:
 * Date                Notes
 * 2020-01-31          the first version
 * 2020-12-09          Modify option type to prevent warning
 * @note: add your note!
...
struct tk_event *tk_event_create(void)
{
    ...
    if ((event = __malloc(sizeof(struct tk_event))) == NULL)
        return NULL;
    event->event_set = 0;
    return event;
}
 ...
 * @return false 接收失败
 */
bool tk_event_recv(struct tk_event *event, const uint32_t event_set, const uint8_t option, uint32_t *recved)
{
    // TK_ASSERT(event);
    bool result = false;
