 ...
 * @LastEditTime: 2025-12-05 21:22:49
 * @version: V1.0.0
 * Change Logs:
 * Date          Notes
 * 2020-01-27    the first version
 * 2020-01-29    add assert for developer
 * 2020-06-04    modify delay_tick type
 * 2020-11-30    fix bug when ticks overflow
 * @note: add your note!
...
#if TK_TIMER_USING_CREATE
    struct tk_timer* tk_timer_node = __malloc(sizeof(struct tk_timer));
...
struct tk_timer* tk_timer_create(void (*timeout_callback)(struct tk_timer* timer))
{
    ...
    if ((timer = __malloc(sizeof(struct tk_timer))) == NULL)
        return NULL;
    timer->enable = false;
    timer->mode = TIMER_MODE_LOOP;
    timer->state = TIMER_STATE_STOP;
...
bool tk_timer_loop_handler(void)
{
    ...
    while (timer != NULL)
    {
        if (timer->enable && (int32_t)(tk_timer_get_tick() - timer->timer_tick_timeout)>=0)
        {
            timer->enable = false;
